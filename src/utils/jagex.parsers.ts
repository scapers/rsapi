...
import {
  ...
  Skill,
  Skills
} from '../api/runescape/player/player.api';
import {
  RawProfile,
  RawSkill
} from '../api/runescape/player/player.models';
...
export class JagexParsers {
  ...
  parseProfile(raw: RawProfile): Profile {
    ...
    raw.skillvalues.forEach((sv: RawSkill) => {
      // find name
      const skillName: string = skillIds.filter(sid => {
        return sid.id === sv.id;
      })[0].name.toLowerCase();
  ...
  private hiscoreToActivities(raw: any): Activities {
    let csv: string[] = raw.split('\n');
    let acts: Activities = new Activities();
