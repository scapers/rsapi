...
const rotationDays = (interval, count, offset, forDate = new Date()) => {
  return {
    rotation: Math.floor(
      ((Math.floor(Math.floor(forDate.getTime() / 1000) / (24 * 60 * 60)) - count) % (interval * count.length)) /
        interval,
    ),
    daysUntilNext:
      interval - (((Math.floor(forDate.getTime() / 1000 / (24 * 60 * 60)) + count) % (interval * count)) % interval),
  };
};
...
export const rots = (forDate: Date = new Date()): Rotation => {
  ...
  const rotsRotations = [
    [
      [names.D, names.T, names.V],
      [names.K, names.A, names.G],
    ],
    [
      [names.K, names.T, names.G],
      [names.A, names.D, names.V],
    ],
    [
      [names.K, names.G, names.V],
      [names.A, names.T, names.D],
    ],
    [
      [names.G, names.T, names.V],
      [names.K, names.A, names.D],
    ],
    [
      [names.K, names.T, names.V],
      [names.A, names.G, names.D],
    ],
    [
      [names.A, names.G, names.D],
      [names.K, names.T, names.V],
    ...
    [
      [names.K, names.A, names.G],
      [names.T, names.D, names.V],
    ],
    [
      [names.A, names.T, names.G],
      [names.K, names.D, names.V],
    ],
    [
      [names.A, names.G, names.V],
      [names.K, names.T, names.D],
    ],
    [
      [names.K, names.A, names.T],
      [names.G, names.D, names.V],
    ],
    [
      [names.K, names.A, names.V],
      [names.D, names.T, names.G],
    ],
    [
      [names.A, names.T, names.V],
      [names.K, names.D, names.G],
    ],
    [
      [names.K, names.D, names.G],
      [names.A, names.T, names.V],
...
export const araxxor = (forDate: Date = new Date()): Rotation => {
  ...
  const paths = [
    { location: 'top', characteristic: 'Minions' },
    { location: 'middle', characteristic: 'Acid' },
    { location: 'bottom', characteristic: 'Darkness' },
  ];
  const rotation = Math.floor(
    ((Math.floor(Math.floor(forDate.getTime() / 1000) / (24 * 60 * 60)) + 3) % (4 * paths.length)) / 4,
  );
...
export const vorago = (forDate: Date = new Date()): Rotation => {
  ...
  const voragoRotations = [rotations.C, rotations.S, rotations.V, rotations.G, rotations.T, rotations.E];
  ...
  const daysUntilNext =
    7 - (((Math.floor(forDate.getTime() / 1000 / (24 * 60 * 60)) - 6) % (7 * voragoRotations.length)) % 7);
